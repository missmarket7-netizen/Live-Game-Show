...
async function loadGeneratedFromTurso() {
  ...
  try {
    ...
    return r.rows.map((row) => ({ category: row.category, difficulty: row.difficulty, question: row.question, options: JSON.parse(row.options), correctIndex: Number(row.correctIndex), explanation: row.explanation, source: row.source || "ai" }));
  } catch (e) { return []; }
}
...
/* ═══ التنظيف: يعالج مفاتيح db1-db5 ذات المسافات الزائدة ═══ */
...
/* ═══ تحميل كل ملفات db*.json من كل المسارات ═══ */
function loadBankQuestions() {
  ...
  for (const dir of dirs) {
    let files = [];
    try {
      files = fs.readdirSync(dir).filter((f) => /^db\d+\.json$/.test(f)).sort((a, b) => parseInt(a.match(/\d+/)[0], 10) - parseInt(b.match(/\d+/)[0], 10));
    } catch (e) { continue; }
    for (const f of files) {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          if (clean.length) { loaded.add(f); all = all.concat(clean); console.log("📚 " + f + " → " + clean.length); }
        }
...
function loadGeneratedFromFile() {
  try {
    ...
    if (fs.existsSync(p)) { const d = JSON.parse(fs.readFileSync(p, "utf8")); if (Array.isArray(d)) return d.map(sanitizeQuestion).filter(Boolean); }
  } catch (e) {}
  return [];
}
...
function extractAndParseJSON(rawText) {
  ...
  if (om) { try { return JSON.parse(om[0]); } catch (e) {} }
...
const PROVIDERS = [
  { name: "gemini", key: GEMINI_API_KEY, call: async (prompt) => {
    ...
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`, { method: "POST", headers: { "Content-Type": "application/json" }, signal: controller.signal, body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: { temperature: 0.9, maxOutputTokens: 4000 } }) });
  ...
  { name: "openrouter", key: OPENROUTER_API_KEY, call: async (prompt) => {
    const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 20000);
    try {
      const res = await fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", signal: controller.signal, headers: { "Content-Type": "application/json", Authorization: `Bearer ${OPENROUTER_API_KEY}`, "HTTP-Referer": "https://live-game-show.app", "X-Title": "Live Game Show" }, body: JSON.stringify({ model: process.env.OPENROUTER_MODEL || "minimax/minimax-m2.7:free", messages: [{ role: "user", content: prompt }], temperature: 0.9, max_tokens: 4000 }) });
      clearTimeout(timeout);
  ...
  { name: "groq", key: GROQ_API_KEY, call: async (prompt) => {
    const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 20000);
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", { method: "POST", signal: controller.signal, headers: { "Content-Type": "application/json", Authorization: `Bearer ${GROQ_API_KEY}` }, body: JSON.stringify({ model: process.env.GROQ_MODEL || "llama-3.3-70b-versatile", messages: [{ role: "user", content: prompt }], temperature: 0.9, max_tokens: 4000 }) });
      clearTimeout(timeout);
...
function getFallbackQuestions(category, count, difficulty) {
  const allQuestions = [
    ...
    { category: "علوم", difficulty: "سهل", question: "ما هو أكبر كوكب في المجموعة الشمسية؟", options: ["الأرض", "المشتري", "زحل", "المريخ"], correctIndex: 1, explanation: "المشتري أكبر الكواكب." },
    { category: "تاريخ", difficulty: "متوسط", question: "من هو فاتح الأندلس؟", options: ["خالد بن الوليد", "طارق بن زياد", "صلاح الدين", "سعد بن أبي وقاص"], correctIndex: 1, explanation: "طارق بن زياد 711م." },
    { category: "رياضة", difficulty: "سهل", question: "كم عدد لاعبي فريق كرة القدم؟", options: ["9", "10", "11", "12"], correctIndex: 2, explanation: "11 لاعباً." },
    { category: "تكنولوجيا", difficulty: "سهل", question: "ما هي شركة آبل؟", options: ["شركة سيارات", "شركة تقنية", "شركة أغذية", "شركة ملابس"], correctIndex: 1, explanation: "شركة تقنية أمريكية." },
...
async function callWithFallback(prompt, count) {
  ...
  for (const provider of PROVIDERS) {
    ...
    try {
      ...
      const valid = questions.map(sanitizeQuestion).filter((q) => q && q.question && Array.isArray(q.options) && q.options.length === 4 && typeof q.correctIndex === "number");
...
/* ═══ الدورة الذكية: مؤشر + خلط دائم في Turso ═══ */
...
app.post("/api/reset-cycle", async (req, res) => {
  try {
    const allBank = await getAllQuestions();
    await setState(0, 1, shuffleArray(allBank.map((_, i) => i)));
...
app.get("/api/cycle-status", async (req, res) => {
  try {
    ...
    res.json({ persistent: Boolean(turso), cursor: st ? st.cursor : 0, cycle: st ? st.cycle_id : 1, bankSize: bank.length });
  } catch (e) {
...
app.get("/api/health", async (req, res) => {
  ...
  res.json({ status: "ok", bankCount: bank.length, turso: Boolean(turso), persistentStorage: Boolean(process.env.RAILWAY_VOLUME_MOUNT_PATH) });
});
...
initTurso().finally(() => {
  app.listen(PORT, () => console.log(`عالم التحديات على ${PORT} | Turso: ${turso ? "✅" : "⚠️"}`));
});
