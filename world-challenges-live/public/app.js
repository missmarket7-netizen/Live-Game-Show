/* ═══════════════════════════════════════════════════════════════════
   🌎 عالم التحديات — LIVE GAME SHOW ENGINE v12 FINAL
   ═══════════════════════════════════════════════════════════════════ */
...
function updateScoreBoxes() {
  ...
  gB.forEach((box, i) => {
    ...
    box.textContent = i < state.girlsScore ? '✓' : '';
  });
  bB.forEach((box, i) => {
    box.classList.toggle('filled-boys', i < state.boysScore);
    box.textContent = i < state.boysScore ? '✓' : '';
  });
...
function minusPoint(team) {
  if (team === 'girls') { if (state.girlsScore > 0) state.girlsScore--; }
...
function showTeamPicker(giftType) {
  $('teamPickerModal').classList.remove('hidden');
...
function initGifts() {
  $$('.gift-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (gift === 'heart') { showTeamPicker('heart'); return; }
      switch(gift) {
        case 'rose':
...
function nextQuestion() {
  // إذا اكتمل فريق 5 نقاط، نعلن النتيجة فوراً
  if (state.girlsScore >= 5 || state.boysScore >= 5) {
    showRoundResult();
    return;
  }
  // إذا انتهت الأسئلة، نخلط البنك الحالي ونكمل (بدون طلب AI جديد)
...
function showRoundResult() {
  ...
  // الأصوات المتسلسلة: الفائز أولاً ثم الخاسر
  playSound('win', 0.6);
  if (winnerSound) {
    setTimeout(() => {
      playSound(winnerSound, 0.7);
      setTimeout(() => { playSound(loserSound, 0.7); }, 2500);
    }, 500);
  } else {
  ...
  $('teamShieldGirls').classList.add('hidden');
  $('teamShieldBoys').classList.add('hidden');
  updateScoreBoxes();
}
...
function generateSingleRound() {
  ...
  showLoading('جاري تحميل الأسئلة من البنك وتوليد الجديد...');
  fetch('/api/questions', {
    method: 'POST',
...
function generateFullShow() {
  ...
  fetch('/api/questions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ count: 200, avoid: state.history.slice(-200) })
  })
  .then(res => res.json())
  .then(data => {
    hideLoading();
...
function init() {
  ...
  // تعديل 1: تشغيل صوت begin بعد 5 ثوانٍ فقط من فتح الصفحة
  setTimeout(() => {
    if (state.soundEnabled) playSound('begin', 0.6);
  }, 5000);
}
