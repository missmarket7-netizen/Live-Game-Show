/* ═══════════════════════════════════════════════════════════════════
   🌎 عالم التحديات — LIVE GAME SHOW ENGINE v11.1 PRO
   ═══════════════════════════════════════════════════════════════════ */
...
function useSavedSet(id) { const entry = getSavedQuestions().find(s => s.id === id); if (!entry || !entry.questions || entry.questions.length === 0) return; state.questions = entry.questions; state.currentIndex = 0; state.girlsScore = 0; state.boysScore = 0; state.girlsRounds = 0; state.boysRounds = 0; state.mode = 'single'; updateScoreBoxes(); showScreen('gameScreen'); renderQuestion(); playSound('begin', 0.6); closeSavedModal(); }
...
/* تعديل 2: الجولة تنتهي فقط باكتمال 5 نقاط أو زر الجولة التالية */
...
/* تعديل 2 و 5: نهاية الجولة تتوقف على حالة النقاط */
function nextQuestion() {
  // إذا اكتمل فريق 5 نقاط، ننتقل للاحتفال
  ...
  if (state.currentIndex < state.questions.length - 1) {
    ...
    // إذا انتهت الأسئلة ولم يكتمل أحد، ننتقل للجولة التالية يدوياً
    showRoundResult();
  }
}
...
function showRoundResult() {
  ...
  if (state.girlsScore > state.boysScore) {
    ...
    playSound('girlsRound'); // صوت الفائز
    setTimeout(() => playSound('boysLose'), 1500); // صوت الخاسر بعده
  } else if (state.boysScore > state.girlsScore) {
    state.boysRounds++;
    winnerText = '🎉 مبروك للشباب!';
    playSound('boysRound'); // صوت الفائز
    setTimeout(() => playSound('girlsLose'), 1500); // صوت الخاسر بعده
  } else {
    winnerText = '🤝 تعادل!';
  }
  
  // عرض الجولة الجديدة في عداد الفريق الفائز
  updateScoreBoxes();
  ...
  // تصفير النقاط والاستعداد للجولة التالية
...
/* تعديل 4: زر توليد أسئلة AI يعمل بشكل صحيح */
...
/* ===== FLOATING SOUND BOARD (المؤثرات في القائمة) ===== */
...
function init() {
  ...
  /* تعديل 1: تشغيل صوت begin بعد 5 ثوانٍ من تحميل الصفحة */
  setTimeout(() => {
