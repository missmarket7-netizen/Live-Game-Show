/* ═══════════════════════════════════════════════════════════════════
   🌎 عالم التحديات — LIVE GAME SHOW ENGINE v12 (نهائي)
   ═══════════════════════════════════════════════════════════════════ */
...
/* إصلاح 7: تشغيل الأصوات بالتتابع وليس بالتوازي */
function playSoundSequence(winnerKey, loserKey) {
  const winAudio = audioCache[winnerKey];
  if (!winAudio) return;
  
  winAudio.onended = () => {
    setTimeout(() => {
      playSound(loserKey, 0.7);
    }, 1000); // ثانية أو ثانيتين بعد انتهاء الأول
  };
  playSound(winnerKey, 0.7);
}
...
window.saveSupporter = function() {
  ...
  if (state.tempSupporterTeam === 'girls') {
    ...
    for (let box of boxes) { if (box.textContent.includes('داعم')) { box.textContent = name; break; } }
...
function nextQuestion() {
  ...
  else { state.questions = shuffleArray([...state.questions]); state.currentIndex = 0; renderQuestion(); }
}
...
/* إصلاح 6 و 7: نتيجة الجولة بالتتابع */
function showRoundResult() {
  ...
  if (state.girlsScore > state.boysScore) {
    ...
    winnerKey = 'girlsRound'; loserKey = 'boysLose';
  } else if (state.boysScore > state.girlsScore) {
    state.boysRounds++;
    winnerText = '🎉 فوز فريق الشباب!';
    winnerKey = 'boysRound'; loserKey = 'girlsLose';
  ...
  if (winnerKey) {
    playSoundSequence(winnerKey, loserKey); // تشغيل بالتتابع
...
/* إصلاح 6: زر النهاية (النتيجة الإجمالية) */
window.showFinalResults = function() {
  stopTimer();
  let winnerText = '';
  let winSound = '';
  if (state.girlsRounds > state.boysRounds) {
    winnerText = '👑 تهنئة للفريق الفائز: البنات!';
    winSound = 'girlsWin';
  } else if (state.boysRounds > state.girlsRounds) {
    winnerText = '👑 تهنئة للفريق الفائز: الشباب!';
    winSound = 'boysWin';
  } else {
    winnerText = '🤝 تعادل مثير!';
    winSound = 'end';
  }
  ...
  setTimeout(() => playSound('celebration', 0.5), 2500);
};

/* إصلاح 3: زر ابدأ الأسئلة (عشوائي بدون تكرار من البنك + AI) */
function startRandomQuestions() {
  const count = Number($('count').value) || 10;
  showLoading('جاري تحميل الأسئلة من البنك...');
  fetch('/api/questions', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ count: count, avoid: state.history.slice(-200) })
  })
  .then(res => res.json())
  .then(data => {
    hideLoading();
    ...
    if (data.questions && data.questions.length > 0) {
      ...
      saveQuestionSet({ id: Date.now(), date: new Date().toLocaleString('ar-SA'), category: 'بنك + AI', difficulty: 'متنوع', count: data.questions.length, questions: data.questions });
      updateScoreBoxes(); showScreen('gameScreen'); renderQuestion();
    }
  }).catch(err => { hideLoading(); alert('خطأ: ' + err.message); });
}

/* إصلاح 2 و 4: زر توليد أسئلة AI */
function generateSingleRound() {
  startRandomQuestions(); // نفس المنطق، جلب من البنك مع توليد تلقائي في السيرفر
}

function generateFullShow() {
  showLoading('جاري تجهيز المسابقة الكاملة...');
  fetch('/api/questions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ count: 200, avoid: state.history.slice(-200) }) })
  .then(res => res.json())
  .then(data => {
    hideLoading();
    if (data.error) throw new Error(data.error);
    ...
    saveQuestionSet({ id: Date.now(), date: new Date().toLocaleString('ar-SA'), category: 'مسابقة', difficulty: 'متنوع', count: allQ.length, questions: allQ });
    updateScoreBoxes(); showScreen('gameScreen'); renderQuestion();
  }).catch(err => { hideLoading(); alert('خطأ: ' + err.message); });
}
/* تهيئة وربط الأزرار */
...
function initSetupUI() {
  $$('.tab').forEach(tab => { tab.addEventListener('click', () => { $$('.tab').forEach(t => t.classList.remove('active')); tab.classList.add('active'); state.mode = tab.dataset.mode; }); });
  $('generateBtn').addEventListener('click', async () => { if (state.mode === 'fullshow') await generateFullShow(); else await generateSingleRound(); });
  $('savedQuestionsBtn').addEventListener('click', startRandomQuestions); // إصلاح 3
...
function initGameUI() {
  ...
  $('nextRoundBtn').addEventListener('click', showRoundResult);
  $('endGameBtn').addEventListener('click', showFinalResults); // إصلاح 6
  $('newRoundBtn').addEventListener('click', () => location.reload());
  $('roundEndContinueBtn').addEventListener('click', continueAfterRound);
  $('closeSavedBtn').addEventListener('click', closeSavedModal);
  $$('.neg-btn').forEach(btn => { btn.addEventListener('click', () => updateNegative(btn.dataset.team, btn.dataset.action)); });
  $('closeSupporterBtn').addEventListener('click', closeSupporterModal);
  $('supporterCancelBtn').addEventListener('click', closeSupporterModal);
  $('supporterModal').addEventListener('click', (e) => { if (e.target.classList.contains('picker-backdrop')) closeSupporterModal(); });
}
...
function init() {
  ...
  // إصلاح 1: بدء الصوت بعد 5 ثوانٍ من فتح الصفحة فقط
