/* ═══════════════════════════════════════════════
   🌎 عالم التحديات — LIVE GAME SHOW ENGINE v3
   ═══════════════════════════════════════════════ */
...
function minusPoint(team) {
  if (team === 'girls') {
    if (state.girlsScore > 0) { state.girlsScore--; playSound('click', 0.3); }
  } else {
    if (state.boysScore > 0) { state.boysScore--; playSound('click', 0.3); }
  }
  updateScoreBoxes();
}
...
function nextQuestion() {
  if (state.currentIndex < state.questions.length - 1) {
    ...
    if (state.mode === 'fullshow' && state.currentRoundIndex < state.fullShowRounds.length - 1) {
      // Check who won this round and record it
      recordRoundWinner();
...
function initSetupUI() {
  ...
  $('savedModal').addEventListener('click', (e) => {
    if (e.target === $('savedModal') || e.target.classList.contains('saved-modal-backdrop')) {
      closeSavedModal();
    }
  });
}
...
function init() {
  ...
  $('soundToggle').addEventListener('click', () => {
    ...
    if (state.soundEnabled) playSound('click', 0.4);
  });
